...
using namespace std;
int IC,PTR,VA,RA,kio=-1,flag[30],PI,TI,SI,OFD, TTC=0,LLC=0;
...
std::ifstream inputFile("input.txt");
    ...
    while (std::getline(inputFile, line)) {
        lineCount++;
        if (line.compare(0, 4, "$DTA") == 0) {
            lineWithDTA = lineCount;
            break;
        }
    }
    ...
    if (lineWithDTA != -1 && lineWithEND != -1 && lineWithEND == lineWithDTA + 1) {
        OFD=1;
        MOS();
